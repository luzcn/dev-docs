                 ...
                 (fn [view-id]
                   [:div
                    [:form {:on-submit (partial e/dispatch-and-cancel
                     ...
                     ;(for [ad (:subnet-error state)]
                     ;  ^{:key (:name ad)}
                     ;  [:div.subfieldset-container
                     ;   [:h3.subfieldset-title (t :subnets.create-title)]
                     ;   [:div.field-pair
                     ;    (if (nil? ad)
                     ;      [:p (t :subnets.confirmation (t :subnets.default-name (:name ad)))]
                     ;      [:p.general-error ad])]])
                        ...
                        (if (m-either/right? vcn-res)
                          (do
                            (when (:expanded @state)
                                  ...
                                  (let [rt-res (<! (ds/update-item
                                                     route-table-store {:rt-id (:defaultRouteTableId (:body @vcn-res))
                                                                        :routeRules [{:cidrBlock "0.0.0.0/0"
                                                                                      :networkEntityType (t :route-rules.default-networkEntityType)
                              ...
                              (let [ch (async/merge (auto-create-subnets vcn-res result ad-list) (count ad-list))]                                
                                (go
                                  (let [subnet-res (<! ch)]
                                      (swap! result assoc :subnet-error "sssss ")
                                      ))
                      ...
                      {:success (fn []
                       ...
                       :error   (fn [result]
                                  (let [err (txr/any-error result)]
                                    (set-error-state! view-id (service-error (:body @err)))))}))
                  )
                db)}})
